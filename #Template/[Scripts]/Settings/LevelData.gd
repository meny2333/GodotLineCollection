...
func get_audio_start_time() -> float:
	var startTime: float = LevelManager.musicCheckpointTime if LevelManager.musicCheckpointTime > 0.0 else LevelManager.animTime
	return startTime if startTime > 0.0 else 0.0
